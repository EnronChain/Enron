...
Learn how Enron handles pending state queries. {synopsis}
...
Enron is designed quite differently on this front as there is no concept of a "pending state".
Enron uses [Tendermint Core](https://docs.tendermint.com/) BFT consensus which provides instant
...
first-in-first-out (FIFO) fashion. Transactions on Enron cannot be ordered or cherry picked out from the Tendermint node [mempool](https://docs.tendermint.com/master/tendermint-core/mempool.html#transaction-ordering).

## Pending State Queries

Enron will make queries which will account for any unconfirmed transactions present in a node's
...
Learn how to deploy a Solidity smart contract on Enron using [Truffle](./../guides/truffle.md) {hide}
