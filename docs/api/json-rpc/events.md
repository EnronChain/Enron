...
Enron also supports the Ethereum [JSON-RPC](https://eth.wiki/json-rpc/API) filters calls to
...
```bash
enrond start --rpc.laddr="tcp://127.0.0.1:26657"
```
...
Since Enron runs uses Tendermint Core as it's consensus Engine and it's built with the Cosmos
...
PubSubAPI](https://geth.ethereum.org/docs/rpc/pubsub), Enron needs to cast the Tendermint
...
```bash
enrond start  --json-rpc.address"0.0.0.0:8545" --json-rpc.ws-address="0.0.0.0:8546" --evm.rpc.api="eth,web3,net,txpool,debug" --json-rpc.enable
```
