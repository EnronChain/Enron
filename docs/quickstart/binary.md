...
`enrond` is the all-in-one command-line interface. It supports wallet management, queries and transaction operations {synopsis}
...
After you have obtained the latest `enrond` binary, run:
...
::: tip
You can also enable auto-completion with the `enrond completion` command. For example, at the start of a bash session, run `. <(enrond completion)`, and all `enrond` subcommands will be auto-completed.
:::

### Config and data directory

By default, your config and data are stored in the folder located at the `~/.enrond` directory.
...
To specify the `enrond` config and data storage directory; you can update it using the global flag `--home <directory>`

### Configuring the Node

The Cosmos SDK automatically generates two configuration files inside `~/.enrond/config`:
...
If you would like to change your nodes pruning strategy then you must do so when the node is initialized. Passing a flag when starting `enron` will always override settings in the `app.toml` file, if you would like to change your node to the `everything` mode then you can pass the `---pruning everything` flag when you call `enrond start`.
...
We can view the default client config setting by using `enrond config` command:
...
For example, the chain identifier can be changed to `enron_9000-3` from a blank name by using:
...
Alternatively, we can directly make the changes to the config values in one place at client.toml. It is under the path of `.enron/config/client.toml` in the folder where we installed enron:
...
After the necessary changes are made in the `client.toml`, then save. For example, if we directly change the chain-id from `enron_{{ $themeConfig.project.testnet_chain_id }}-2` to `enrontest_9000-1`, and output to number, it would change instantly as shown below.

```bash
enrond config
{
 "chain-id": "enrontest_9000-1",
 "keyring-backend": "os",
 "output": "number",
 "node": "tcp://localhost:26657",
 "broadcast-mode": "sync"
...
A list of commonly used flags of `enrond` is listed below:

| Option              | Description                   | Type                                             | Default Value |
| ------------------- | ----------------------------- | ------------------------------------------------ | ------------- |
| `--chain-id`        | Full Chain ID                 | `string`                                         | `""`          |
| `--home`            | Directory for config and data | `string`                                         | `~/.enrond`   |
| `--keyring-backend` | Select keyring's backend      | `{"os"|"file"|"kwallet"|"pass"|"test"|"memory"}` | `"os"`        |
| `--output`          | Output format                 | `string`                                         | "text"        |

## Command list

A list of commonly used `enrond` commands. You can obtain the full list by using the `enrond -h` command.
